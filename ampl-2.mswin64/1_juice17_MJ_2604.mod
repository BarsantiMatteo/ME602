...
var area_step1 >= 0.001;		   # heat exchange area spary cooling STEP1
var area_step2 >= 0.001;		   # heat exchange area spary cooling STEP1
...
var capex_step1_coolant;	   # CAPEX of 2nd additonal heat exchanger using coolant from STEP1
var capex_step2_coolant;	   # CAPEX of 1st additonal heat exchanger using coolant from STEP2
...
cp_juice * (Ttarget - Tinter_step1) * m1 * p_juice = cp_juice * (T4 - T3) * m1 * p_juice;
...
area_interm_coolant = cp_juice * (T4 - T3) * m1 * p_juice / (Tm_interim_boiler * Uref);



subject to mixing_bottles: # Energy balance where juice is poured into the bottles
cp_juice * m1 * p_juice * (Tinter_step1 - Tin_step1) = m_empty_bottles * bps * (Tin_step1 - Tempty_bottles) * cp_empty_bottles;
...
capex = capex_step1 + capex_step2 + capex_refrigeration + capex_boiler + capex_step1_coolant + capex_step2_coolant + capex_interm_cool;
